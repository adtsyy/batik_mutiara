        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <div class="text-2xl font-semibold text-gray-900 mt-3">
                        {{ format_rupiah($monthRevenue) }}
                    </div>
                    <div class="text-xs text-gray-500 mt-1">{{ \Carbon\Carbon::now()->locale('id')->translatedFormat('F Y') }}</div>
                </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <div class="text-2xl font-semibold text-gray-900 mt-3">
                        {{ format_rupiah($yearRevenue) }}
                    </div>
                    <div class="text-xs text-gray-500 mt-1">{{ \Carbon\Carbon::now()->year }}</div>
                </div>
                ...
                <div class="flex items-center justify-between border-b py-4 last:border-b-0">
                    <div>
                        ...
                        <div class="text-xs text-gray-500 mt-1">
                            {{ $sale->cashierName }} • {{ \Carbon\Carbon::parse($sale->created_at)->translatedFormat('d/m/Y, H.i.s') }}
                        </div>
                    </div>
                    <div class="text-right">
                        <div class="text-sm font-semibold text-green-600">{{ format_rupiah($sale->total) }}</div>
